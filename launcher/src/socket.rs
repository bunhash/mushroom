...
pub(crate) unsafe fn main() {
    let ip = CString::new(IP).unwrap_or_else(|e| {
        winlog!("[socket::main] ERROR: {:?}", e);
        unsafe { ExitProcess(3424) };
        panic!();
    });
    *REROUTED_ADDR.lock().unwrap_or_else(|e| {
        winlog!("[socket::main] ERROR: {:?}", e);
        unsafe { ExitProcess(3424) };
        panic!();
    }) = inet_addr(ip.as_ptr());
    let address = utils::load_module_symbol("mswsock.dll", "WSPStartup").unwrap_or_else(|e| {
        winlog!("[socket::main] ERROR: {:?}", e);
        unsafe { ExitProcess(3424) };
        panic!();
    });
